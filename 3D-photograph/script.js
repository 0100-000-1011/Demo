...
function drop() {
    ...
    for (var i = 0; i < ps.length; i++) {
        ps[i].index = i;
        ps[i].ondblclick = function (e) {
            ps[this.index].style.backgroundImage = '';
            local[this.index] = '';
            ps[this.index].innerHTML = inner;
            localStorage.ps = JSON.stringify (local);
        }
...
function generate (num = 1) {
    var dz = document.querySelector ('div.z');
    for (let i = 0; i < num; ++i)
        dz.appendChild (document.createElement ('p'));
}
var totnum = 12;
generate (totnum);
var inner = '可以拖拽图片文件进来';
var local = new Array ();
if (localStorage.ps)
    local = JSON.parse (localStorage.ps);
{
    ...
    for (let i = 0; i < len; ++i) {
        ...
        if (!img || img == undefined || img == '') {
            ps[i].innerHTML = inner;
            continue;
        }
        ps[i].style.backgroundImage = img;
        ps[i].innerHTML = '';
    }
    for (let i = len; i < totnum; ++i)
        ps[i].innerHTML = inner;
}